...
def run_lorentzian_backtest(params=None):
    import pandas as pd
    from advanced_ta import LorentzianClassification
    from backtesting import Backtest, Strategy
    import warnings
    ...
    class LorentzianStrategy(Strategy):
        neighborsCount = params.get('neighborsCount', 8)
        commission = params.get('commission', 0.002)
        maxBarsBack = params.get('maxBarsBack', 1500)
        useDynamicExits = params.get('useDynamicExits', True)
        useVolatilityFilter = params.get('useVolatilityFilter', True)
        useRegimeFilter = params.get('useRegimeFilter', True)
        useAdxFilter = params.get('useAdxFilter', True)
        regimeThreshold = params.get('regimeThreshold', 0)
        adxThreshold = params.get('adxThreshold', 10)
        useKernelSmoothing = params.get('useKernelSmoothing', True)
        lookbackWindow = params.get('lookbackWindow', 10)
        relativeWeight = params.get('relativeWeight', 4.0)
        regressionLevel = params.get('regressionLevel', 10)
        crossoverLag = params.get('crossoverLag', 1)
        useEmaFilter = params.get('useEmaFilter', True)
        emaPeriod = params.get('emaPeriod', 50)
        useSmaFilter = params.get('useSmaFilter', True)
        smaPeriod = params.get('smaPeriod', 50)
        num_Bars_var = params.get('num_Bars_var', 2)
    ...
    bt = Backtest(bt_df, LorentzianStrategy, cash=10000, commission=LorentzianStrategy.commission)
    stats = bt.run()
    trades = []
    for _, row in bt.trades.iterrows():
        trades.append({
            'date': str(row['EntryTime']),
            'action': 'BUY' if row['Size'] > 0 else 'SELL',
            'price': float(row['EntryPrice']),
            'pnl': float(row['PnL ($)'])
        })
    result = {
        'profit_loss': float(stats['Equity Final [$]'] - 10000),
        'trades': trades,
        'stats': {
...
def run_lorentzian_optimize(params=None):
    import pandas as pd
    from advanced_ta import LorentzianClassification
    from backtesting import Backtest, Strategy
    import warnings
