...
class LorentzianClassification:
    """
    Refactored LorentzianClassification to match PineScript flexibility and parameterization.
    """
    def __init__(self, data: pd.DataFrame,
        ...
        """
        feature_defs: list of (feature_type:str, paramA:int, paramB:int)
        """
        self.df = data.copy()
        self.log = logging.getLogger("LorentzianClassification")
        ...
        self.kernel_filter = kernel_filter or KernelFilter()
        self.features = []
        self.yhat1 = None
        self.yhat2 = None

        # Defaults matching PineScript
        ...
        self.feature_defs = feature_defs[:feature_count]
    ...
    def _build_features(self):
        self.features = []
        for ftype, pA, pB in self.feature_defs:
            if ftype == "RSI":
                self.features.append(ml.n_rsi(self.df['close'], pA, pB))
            elif ftype == "WT":
                hlc3 = (self.df['high'] + self.df['low'] + self.df['close']) / 3
                self.features.append(ml.n_wt(hlc3, pA, pB))
            elif ftype == "CCI":
                self.features.append(ml.n_cci(self.df['high'], self.df['low'], self.df['close'], pA, pB))
            elif ftype == "ADX":
                self.features.append(ml.n_adx(self.df['high'], self.df['low'], self.df['close'], pA))
            else:
                raise ValueError(f"Unknown feature type: {ftype}")
        self.features = [np.array(f) for f in self.features]

    def _build_filters(self):
        ohlc4 = (self.df['open'] + self.df['high'] + self.df['low'] + self.df['close']) / 4
        self.filter = Filter(
            volatility = ml.filter_volatility(self.df['high'], self.df['low'], self.df['close'], self.use_volatility_filter, 1, 10),
            regime = ml.regime_filter(ohlc4, self.df['high'], self.df['low'], self.use_regime_filter, self.regime_threshold),
            adx = ml.filter_adx(self.df['close'], self.df['high'], self.df['low'], self.adx_threshold, self.use_adx_filter, 14)
        )

    def _lorentzian_distance(self, idx, feature_vec, feature_matrix):
        # idx: int, feature_vec: array, feature_matrix: list of arrays
        # Returns Lorentzian distance between feature_vec and each row in feature_matrix
        dist = np.zeros(len(feature_matrix[0]))
        for i, f in enumerate(feature_matrix):
            dist += np.log(1 + np.abs(feature_vec[i] - f))
        return dist

    def __classify(self):
        maxBarsBackIndex = max(0, len(self.df.index) - self.max_bars_back)
        src = self.df['close']
        # Trend filters
        isEmaUptrend = np.where(self.use_volatility_filter, (self.df["close"] > EMA(self.df["close"], 14)), True)
        isEmaDowntrend = np.where(self.use_volatility_filter, (self.df["close"] < EMA(self.df["close"], 14)), True)
        isSmaUptrend = np.where(self.use_regime_filter, (self.df["close"] > SMA(self.df["close"], 14)), True)
        isSmaDowntrend = np.where(self.use_regime_filter, (self.df["close"] < SMA(self.df["close"], 14)), True)

        # Prepare training labels (future return direction)
        y_train_array = np.where(src.shift(-4) < src, Direction.SHORT, np.where(src.shift(-4) > src, Direction.LONG, Direction.NEUTRAL))

        predictions = np.zeros(len(src))
        for bar_index in range(maxBarsBackIndex, len(src)):
            # Build feature vector for current bar
            feature_vec = [f[bar_index] for f in self.features]
            # Build matrix of historical features
            feature_matrix = [f[maxBarsBackIndex:bar_index] for f in self.features]
            if len(feature_matrix[0]) == 0:
                predictions[bar_index] = 0
                continue
            dists = np.zeros(len(feature_matrix[0]))
            for i in range(len(feature_matrix[0])):
                dists[i] = np.sum([np.log(1 + np.abs(feature_vec[j] - feature_matrix[j][i])) for j in range(len(self.features))])
            # Find k nearest neighbors
            k = min(self.neighbors_count, len(dists))
            if k == 0:
                predictions[bar_index] = 0
                continue
            nn_idx = np.argpartition(dists, k)[:k]
            neighbor_labels = y_train_array[maxBarsBackIndex:bar_index][nn_idx]
            pred = np.sum(neighbor_labels)
            predictions[bar_index] = pred
            self.log.debug(f"bar {bar_index}, pred {pred}, neighbors {neighbor_labels}")

        # Filtering and signal logic
        filter_all = self.filter.volatility & self.filter.regime & self.filter.adx
        signal = np.where(((predictions > 0) & filter_all), Direction.LONG, np.where(((predictions < 0) & filter_all), Direction.SHORT, None))
        signal[0] = (0 if signal[0] is None else signal[0])
        for i in np.where(signal == None)[0]:
            signal[i] = signal[i - 1 if i >= 1 else 0]

        # Entry/exit logic (simplified, can be expanded for full PineScript parity)
        isDifferentSignalType = (signal != np.roll(signal, 1))
        isNewBuySignal = (signal == Direction.LONG) & isDifferentSignalType & isEmaUptrend & isSmaUptrend
        isNewSellSignal = (signal == Direction.SHORT) & isDifferentSignalType & isEmaDowntrend & isSmaDowntrend
        self.df['prediction'] = predictions
        self.df['signal'] = signal
        self.df['startLongTrade'] = np.where(isNewBuySignal, self.df['low'], np.nan)
        self.df['startShortTrade'] = np.where(isNewSellSignal, self.df['high'], np.nan)
        # TODO: Add dynamic and strict exit logic as in PineScript
        self.log.info(f"Total Long Entries: {np.nansum(isNewBuySignal)} | Total Short Entries: {np.nansum(isNewSellSignal)}")
    ...
    def plot(self, name: str):
        ...
        length = self.df.index.size
        sub_plots = [
            mpf.make_addplot(self.df['prediction'][:length], ylabel="Prediction", color='blue'),
            mpf.make_addplot(self.df["startLongTrade"][:length], ylabel="Long", color='green', type='scatter', markersize=120, marker='^'),
            mpf.make_addplot(self.df["startShortTrade"][:length], ylabel="Short", color='red', type='scatter', markersize=120, marker='v'),
        ]
        ...
        fig, axlist = mpf.plot(self.df[['open', 'high', 'low', 'close']].head(length), type='candle', style=s, addplot=sub_plots, figsize=(30, 40), returnfig=True)
    ...
    def __classify(self):
        ...
        # If you want to use EMA/SMA filters, add corresponding attributes to __init__ and pass them in
        isEmaUptrend = np.where(getattr(self, 'use_ema_filter', False), (self.df["close"] > EMA(self.df["close"], getattr(self, 'ema_period', 14))), True)
        isEmaDowntrend = np.where(getattr(self, 'use_ema_filter', False), (self.df["close"] < EMA(self.df["close"], getattr(self, 'ema_period', 14))), True)
        isSmaUptrend = np.where(getattr(self, 'use_sma_filter', False), (self.df["close"] > SMA(self.df["close"], getattr(self, 'sma_period', 14))), True)
        isSmaDowntrend = np.where(getattr(self, 'use_sma_filter', False), (self.df["close"] < SMA(self.df["close"], getattr(self, 'sma_period', 14))), True)
        ...
        def get_lorentzian_predictions():
            ...
            for bar_index in range(maxBarsBackIndex, len(src)):
                lastDistance = -1.0
                span = min(self.max_bars_back, bar_index + 1)
        ...
        isBullish = np.where(self.use_kernel_filter, np.where(kFilter.useKernelSmoothing, isBullishSmooth, isBullishRate), True)
        isBearish = np.where(self.use_kernel_filter, np.where(kFilter.useKernelSmoothing, isBearishSmooth, isBearishRate) , True)
