...
class MyStrategy(Strategy):
    def init(self):
        ...
        self.classifier = LorentzianClassification(data=self.data)

    def next(self):
        # Generate signals based on your classifier
        if self.classifier.predict_signal() == 'BUY':
            self.buy()
        elif self.classifier.predict_signal() == 'SELL':
            self.sell()

# Download historical price data
spy_daily_data = yf.download('TSLA', period="5y")

# Ensure columns are properly named for Backtest
if isinstance(spy_daily_data.columns, pd.MultiIndex):
    spy_daily_data.columns = ['_'.join(col).strip() for col in spy_daily_data.columns.values]
spy_daily_data.columns = [c.capitalize() for c in spy_daily_data.columns]
required_cols = ['Open', 'High', 'Low', 'Close', 'Volume']
spy_daily_data = spy_daily_data[[col for col in required_cols if col in spy_daily_data.columns]]

# Instantiate the Backtest class with your strategy and data
bt = Backtest(data=spy_daily_data, strategy=MyStrategy, cash=100000, commission=0.002)
...
print(results)
print(results)
missing = [col for col in required_cols if col not in spy_daily_data.columns]
if missing:
    print("DEBUG: Columns after renaming:", list(spy_daily_data.columns))
    raise ValueError(f"Missing columns for Backtest: {missing}. Columns in data: {list(spy_daily_data.columns)}")
spy_daily_data = spy_daily_data[required_cols]
